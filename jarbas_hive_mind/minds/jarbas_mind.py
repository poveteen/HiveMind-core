...
class JarbasServerFactory(WebSocketServerFactory):
    ...
    def register_internal_messages(self):
        ...
        self.emitter.on('hive.client.broadcast', self.handle_broadcast)
        self.emitter.on('hive.client.send', self.handle_send)
    ...
    def shutdown(self):
        self.emitter.remove('message', self.handle_message)
        self.emitter.remove('hive.client.broadcast', self.handle_broadcast)
        self.emitter.remove('hive.client.send', self.handle_send)


def start_mind(config=None, emitter=None):
    # server
    config = config or Configuration.get().get("hivemind", {}).get("master mind", {})
    ...
    factory = JarbasServerFactory(adress, emitter=emitter)
